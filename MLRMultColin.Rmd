...
```{r}
teams = teams %>% mutate(resid1 = model1$residuals)
```

We create separate residual plots for each of the three predictor variables in our model.  
```{r}
p1 = ggplot(teams, aes(x=R,y=resid1)) + geom_point() + theme_bw()
p2 = ggplot(teams, aes(x=H,y=resid1)) + geom_point() + theme_bw()
p3 = ggplot(teams, aes(x=HR,y=resid1)) + geom_point() + theme_bw()
grid.arrange(p1,p2,p3, ncol = 2) #arranging ggplot objects in a grid
```
```{r}
ggplot(teams,aes(x=resid1)) + geom_histogram() + theme_bw()
```
The residual plos are not indicative of unequl variance or non-Normal residuals.  
